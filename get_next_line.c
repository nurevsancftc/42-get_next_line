...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/29 15:13:30 by nuciftci          #+#    #+#             */
/*   Updated: 2024/11/29 16:11:13 by nuciftci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	readone = read_line(fd, readone);
	if (!readone)
		return (NULL);
	arr = put_line(readone);
	readone = nxt_line(readone);
	return (arr);
}
